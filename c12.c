...
// Função de comparação para bolha (date_added, desempate por title)
int compareShowsBubble(const void *a, const void *b) {
...
void bubbleSort(Show *arr, int n, int *comparisons) {
  for (int i = 0; i < n - 1; i++) {
    for (int j = 0; j < n - i - 1; j++) {
      (*comparisons)++;
      if (compareShowsBubble(&arr[j], &arr[j + 1]) > 0) {
        Show temp = arr[j];
        arr[j] = arr[j + 1];
        arr[j + 1] = temp;
      }
...
int main() {
  ...
  clock_t start = clock();
  int comparisons = 0;
  bubbleSort(selecionados, countSelecionados, &comparisons);
  clock_t end = clock();

  for (int i = 0; i < countSelecionados; i++) printShow(&selecionados[i]);

  FILE *log = fopen("866308_bolha.txt", "w");
  if (log) {
